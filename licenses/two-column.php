...
function ct_tracks_two_column_sanitize_license( $new ) {
    $old = get_option( 'ct_tracks_two_column_license_key' );
    if( $old && $old != $new ) {
        delete_option( 'ct_tracks_two_column_license_key_status' ); // new license has been entered, so must reactivate
    }
    return $new;
}
...
function ct_tracks_two_column_activate_license() {

    if( isset( $_POST['ct_tracks_two_column_license_activate'] ) ) {
        ...
        update_option( 'ct_tracks_two_column_license_key_status', $license_data->license );

    }
}
add_action('admin_init', 'ct_tracks_two_column_activate_license');
...
function ct_tracks_two_column_deactivate_license() {

    // listen for our activate button to be clicked
    if( isset( $_POST['ct_tracks_two_column_license_deactivate'] ) ) {
        ...
        if( $license_data->license == 'deactivated' )
            delete_option( 'ct_tracks_two_column_license_key_status' );

    }
}
add_action('admin_init', 'ct_tracks_two_column_deactivate_license');
...
function ct_tracks_two_column_check_license() {
